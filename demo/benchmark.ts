...
const std1 = Math.sqrt(results['vite-react-svg'].reduce((acc, val) => acc + Math.pow(val - avg1, 2), 0) / samples);
const std2 = Math.sqrt(results['vite-plugin-svgr'].reduce((acc, val) => acc + Math.pow(val - avg2, 2), 0) / samples);

console.log('\nResults:');
console.log('vite-react-svg:', avg1.toFixed(2), '±', std1.toFixed(2), 'ms');
console.log('vite-plugin-svgr:', avg2.toFixed(2), '±', std2.toFixed(2), 'ms');
    console.log(`Relative speed: vite-react-svg is ${(avg2/avg1).toFixed(2)}x faster than vite-plugin-svgr`);
...
console.log('vite-plugin-svgr output size:', comp2 ? comp2.code.length : 0, 'bytes');
